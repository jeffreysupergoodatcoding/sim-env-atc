...
// Configuration
const CONFIG = {
    defaultCenter: [37.4611, -122.1151],
    defaultZoom: 12,
...
document.addEventListener('alpine:init', () => {
    Alpine.store('atc', {
        ...
        demoPipeline: {
            ...
            verification: null,
            response: '',
            status: 'idle' // idle, active, extraction, verification, response
        },

        pipelineLogs: [], // Array of logged pipeline runs for RLHF
        ...
        simulationModal: {
            lat: 37.4611, // Default to KPAO area
            lon: -122.1151,
            altitude: 5000,
        ...
        setSimulationPositionFromMap() {
            ...
            if (this.simulationModal.mapClickMode) {
                ...
                if (this.mapManager) this.mapManager.disableSimulationPositionMode();
            }
        },
        ...
        async initDemo() {
            ...
            if (!window.demoService) {
                console.error('[Demo] window.demoService not found');
                return;
            }

            // Wait for demo service to initialize
            await window.demoService.init();
            this.demoMode = window.demoService.demoMode;
            this.demoClips = window.demoService.clips;

            // Listen for demo state updates
            document.addEventListener('demo-state-updated', () => {
                this.demoCurrentIndex = window.demoService.currentIndex;
                this.demoIsPlaying = window.demoService.isPlaying;
            });
        },

        generateRandomPosition() {
            // Generate random position within 50 nautical miles of airport
            const centerLat = this.stationLatitude || 37.4611; // KPAO default
            const centerLon = this.stationLongitude || -122.1151;
        ...
        async fetchStationData() {
            try {
                // Also fetch public config to sync demo mode
                const cfgResponse = await fetch('/api/v1/config');
                if (cfgResponse.ok) {
                    const cfg = await cfgResponse.json();
                    this.demoMode = cfg.demo_mode === true;
                    console.log('[Alpine Store] Demo mode synced:', this.demoMode);
                }
                ...
                if (!response.ok) {
                    ...
                    this.stationLatitude = 37.4611; // Default fallback
                    this.stationLongitude = -122.1151; // Default fallback
                    this.stationElevationFeet = 4; // Default fallback
                    return;
                }
                const data = await response.json();
                ...
                // Fallback to some default if needed, or handle error appropriately
                this.stationLatitude = 37.4611; // Default fallback on error
                this.stationLongitude = -122.1151; // Default fallback on error
